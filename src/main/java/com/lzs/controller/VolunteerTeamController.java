...
public class VolunteerTeamController extends BaseController{
	...
	public ReturnResult queryVolunteerTeamList(String name,String type,int pageSize,int pageNum){
		Map<String,Object> paramMap = new HashMap<String,Object>();
	 ...
	 * 查询志愿者团队管理人员管理的志愿者团队
	 */
	@RequestMapping("/queryVolunteerTeamListForManager")
	public ReturnResult queryVolunteerTeamListForManager() {
		User user = this.getUser();
		String user_id = user.getUuid();
		int role = user.getRole();
		if(role!=2) {
			return ReturnResult.error("不是志愿者团队管理员");
		}
		List<Map<String,Object>> list = vTeamUserService.queryVolunteerTeamListForManager(user_id);
