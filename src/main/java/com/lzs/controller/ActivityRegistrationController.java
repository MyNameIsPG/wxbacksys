...
public class ActivityRegistrationController extends BaseController{
	 ...
	 * 查询活动是否已报名
	 */
	@RequestMapping("/queryActivityRegistrationForUser")
	public ReturnResult queryActivityRegistrationForUser(String activity_id){
		if(StringUtils.isEmpty(activity_id)){
			return ReturnResult.error("活动id不能为空！");
		}
		User user = this.getUser();
		...
		int size = activityRegistrationService.queryActivityRegistrationForUser(paramMap);
